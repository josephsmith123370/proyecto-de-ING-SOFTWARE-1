 ...
 * ============================================================
 * admin-login.js
 * Lógica del panel de acceso administrativo
 * ============================================================
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ===== CONFIGURACIÓN =====
    const API_LOGIN = 'http://localhost:3000/api/admin/login'
    const API_VERIFY = 'http://localhost:3000/api/admin/verify-otp'
    const API_RESEND = 'http://localhost:3000/api/admin/resend-otp'

    // Estado global
    let adminCorreoActual = null;
        ...
        formLogin.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (datos.exito && datos.requireOTP) {
                    // Mostrar modal OTP
                    adminCorreoActual = datos.correo
                    otpModal.style.display = 'flex'
                    setLoadingState(false)
                } else if (datos.exito && !datos.requireOTP) {
                    // Fallback por si no usa OTP (no debería ocurrir según el nuevo flujo)
        ...
        btnVerifyOTP.addEventListener('click', async () => {
            ...
            try {
                const respuesta = await fetch(API_VERIFY, {
                    ...
                    body: JSON.stringify({ correo: adminCorreoActual, codigo_otp })
                })
                const datos = await respuesta.json()
        ...
        btnResendOTP.addEventListener('click', async () => {
            ...
            try {
                const respuesta = await fetch(API_RESEND, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ correo: adminCorreoActual })
                })
                const datos = await respuesta.json()
    ...
    function setLoadingState(isLoading) {
        if (isLoading) {
            ...
            btnText.textContent = 'Iniciar Sesión Administrativa'
