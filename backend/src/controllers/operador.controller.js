/**
 * ============================================================
 * operador.controller.js
 * Sistema de Gestión Inteligente de Residuos Sólidos - Wanchaq
 * 
 * Controlador de operadores - Maneja registro y login
 * con verificación de OTP.
 * ============================================================
 */
 ...
 * 
 * @route POST /api/operador/registro
 * @param {Object} req.body - { nombre, dni, correo, password }
 * @returns {Object} JSON con mensaje de éxito y datos del operador
 */
export const registrarOperador = async (req, res) => {
    ...
    try {
        ...
        // ===== Insertar nuevo operador en Supabase =====
            ...
            .insert([
                {
                    ...
                    password_hash,
                    rol: 'operador' // Asignación explícita del rol de operador
                }
            ])
            .select()

        if (error) {
            console.error('Error de Supabase al registrar operador:', error)
            return res.status(400).json({
                exito: false,
                mensaje: 'Error al registrar operador en la base de datos'
            })
        }
        ...
        console.error('Error en registrarOperador:', error)
        res.status(500).json({
            exito: false,
 ...
 * 
 * @route POST /api/operador/login
 * @param {Object} req.body - { correo, password }
 * @returns {Object} JSON con petición de OTP
 */
export const loginOperador = async (req, res) => {
    ...
    try {
        ...
        if (usuario.rol !== 'operador') {
            return res.status(403).json({
                exito: false,
                mensaje: 'Esta cuenta no pertenece a un operador'
            })
        }
        ...
        console.error('Error en loginOperador:', error)
        res.status(500).json({
            exito: false,
            mensaje: 'Error interno del servidor'
        })
 ...
 * 
 * @route POST /api/operador/verify-otp
 * @param {Object} req.body - { correo, codigo_otp }
 */
export const verifyOTPOperador = async (req, res) => {
    ...
    try {
        ...
        console.log('--- AUDITORÍA OTP OPERADOR ---')
        ...
        console.error('Error en verifyOTPOperador:', error)
 ...
 * 
 * @route POST /api/operador/resend-otp
 * @param {Object} req.body - { correo }
 */
export const resendOTPOperador = async (req, res) => {
    ...
    try {
        ...
        console.error('Error en resendOTPOperador:', error)
